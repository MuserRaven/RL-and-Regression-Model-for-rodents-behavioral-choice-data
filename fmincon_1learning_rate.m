...
%% Optimization
% For 1 learning rate group
param_table_ACC = table([],[],[],[],[],[],[],[],[],[],[],[],[],[],'VariableNames', {'RatID','Sex','Expression','Group','Drug','Brain Area', 'Session', 'Alpha', 'R', 'Beta','bias','Gamma','MI_M(information decay)','Loss'});
param_table_control = table([],[],[],[],[],[],[],[],[],[],[],[],[],[],'VariableNames', {'RatID','Sex','Expression','Group','Drug','Brain Area', 'Session', 'Alpha', 'R', 'Beta','bias','Gamma','MI_M(information decay)','Loss'});
param_table_BLA = table([],[],[],[],[],[],[],[],[],[],[],[],[],[],'VariableNames', {'RatID','Sex','Expression','Group','Drug','Brain Area', 'Session', 'Alpha', 'R', 'Beta', 'bias','Gamma','MI_M(information decay)','Loss'});
param_table_OFC = table([],[],[],[],[],[],[],[],[],[],[],[],[],[],'VariableNames', {'RatID','Sex','Expression','Group','Drug','Brain Area', 'Session', 'Alpha', 'R', 'Beta','bias','Gamma','MI_M(information decay)','Loss'});
...
%             concat_choice_type = [];
            ...
            if patch == 1
                ...
                [sorted_trial_N, sort_idx] = sort(trial_N_values); % sort trial index because they are not arranged well.
            ...
            objective = @(x) piece_wise_lossfunction(x(1), 0, x(2), x(3), x(4),0, rat_choice, rat_reward, choice_type, sound_cue);
            % Lower and upper bounds for alpha, r, and beta
            lb = [eps, eps, -1000,eps];  % change to the lower bounds suitable for your problem
            ub = [1, 100, 1000, 1];  % change to the upper bounds suitable for your problem
            ...
            loss = piece_wise_lossfunction(x(1),0, x(2), x(3), x(4),0, rat_choice,rat_reward, choice_type, sound_cue);

            if patch == 1
                param_table_ACC = [param_table_ACC; {rat_id,sex, expression,group,drug, brain_area, session, x(1), 0, x(2), x(3), x(4),0,loss}];
            elseif patch == 2
                param_table_control = [param_table_control; {rat_id,sex, expression,group,drug, brain_area, session, x(1), 0, x(2), x(3), x(4),0,loss}];
            elseif patch == 3
                param_table_BLA = [param_table_BLA; {rat_id,sex, expression,group,drug, brain_area, session, x(1), 0, x(2), x(3), x(4),0,loss}];
            elseif patch == 4
                param_table_OFC = [param_table_OFC; {rat_id,sex, expression,group,drug, brain_area, session, x(1), 0, x(2), x(3), x(4),0,loss}];
            end
...
loss_ACC = param_table_ACC.Loss;
loss_BLA = param_table_BLA.Loss;
loss_control = param_table_control.Loss;
loss_OFC = param_table_OFC.Loss;
loss_ACC_2lr = param_table_ACC_2lr.Loss;
loss_BLA_2lr = param_table_BLA_2lr.Loss;
loss_control_2lr = param_table_control_2lr.Loss;
loss_OFC_2lr = param_table_OFC.Loss;

% Concatenate the data from all tables
loss_all = [loss_ACC; loss_BLA; loss_control; loss_OFC];
loss_all_2lr = [loss_ACC_2lr; loss_BLA_2lr; loss_control_2lr; loss_OFC_2lr];
...
%kruskalwallis( [loss_all, loss_all_2lr],[1,2]);


%% No bootstrap
tables = {param_table_ACC, param_table_BLA, param_table_OFC, param_table_control};
labels = {'ACC', 'BLA', 'OFC', 'Control'};
n_bootstrap = 1000;
cols_of_interest = [8,10,11,12];
...
cols_of_interest = 8:11;

% Extract the columns of interest from each table
ACC_data = param_table_ACC(:, cols_of_interest);
BLA_data = param_table_BLA(:, cols_of_interest);
OFC_data = param_table_OFC(:, cols_of_interest);
Control_data = param_table_control(:, cols_of_interest);
...
% Re-order the tables and labels to ACC, BLA, OFC, Control
tables = {param_table_ACC, param_table_BLA, param_table_OFC, param_table_control};
labels = {'ACC', 'BLA', 'OFC', 'Control'};
...
colors = {[1, 0, 0], [0, 0, 1], [0, 0.75, 0.75], [0, 1, 0]};  % Adjusted color for 'Control' condition to green
...
for j = 1:numel(cols_of_interest)
    ...
    for i = 1:numel(tables)
        ...
        for h = 1:n_bootstrap
            ...
            bs_cv = cat(1,bs_cv,std(sample)/mean(sample));
        ...
        rectangle('Position',[i-0.25 bs_25 0.5 bs_75-bs_25], 'FaceColor', [colors{i},1], 'EdgeColor', 'k');  % Now with black edge
        % Line for mean
